...
- **Windows Server Active Directory (AD) domain controllers**. In this reference achitecture we are deploying Windows Server AD domain controllers. These domain controllers run in the Azure VNet and have a trust relationship with the on-premises Windows Server AD forest. For reference, SharePoint Server does supprt Azure Active Directory Domain Services so the architecture can be adapted to implement that if preferred. Client web requests for SharePoint farm resources are authenticated in the VNet rather than sending that authentication traffic across the gateway connection to the on-premises network. In DNS, intranet A or CNAME records are created so that intranet users can resolve the name of the SharePoint farm to the private IP address of the internal load balancer.
